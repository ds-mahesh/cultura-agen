...
const Location: Template<ExternalApiRenderData> = ({
  ...
  const KidsandEaster = c_kidsandEaster?.map((link: any) => (

    <a className="w-full" href={link.cardcta.link} >
      <div className="w-full">
        <img className="h-96 w-full" src={link.cardimage.url} />
      </div>
      <div className="pl-6 -mt-64 pb-24" style={{marginTop:"-300px"}}>
        <h2 className="font-medium text-xl">{link.cardTitle}</h2>
        <p>Until {link.cardDate}</p>
        <h1 className="pt-12 font-bold">{link.cardHeading}</h1>
        <p className="text-xl font-medium pt-12 pb-6">{link.carddescription}</p>
        <button className="rounded-lg px-4 py-4 border border-indigo-600">{link.cardcta.label}</button><br />
      </div>
    </a>
  ...
  const Join = c_join?.map((link: any) => (
    <a className="" href={link.link} >
      <button className="rounded-full px-4 py-2 border border-indigo-600">{link.label}</button><br />
    </a>
  ));
      ...
      >
        ...
        <AnalyticsScopeProvider name={""}>
          <PageLayout global={_site}>
            ...
            <div className="flex space-x-9  pl-20 pr-20">
              {KidsandEaster}
            </div>
            <div className="pt-6">
              {Registernowbgimage}
              ...
              <a className="flex justify-center pt-16" href={c_registernowctaimage.cta.link}>
                <button className="rounded-lg px-4 py-4 border border-indigo-600">{c_registernowctaimage.cta.label}</button>
              </a>
            </div>
            <div className="pl-24 pt-36">
              <div className="text-xl font-semibold">{c_notToBeMissed}</div>
              <div className="font-bold text-5xl text-gray-800 pt-4">{c_happensatculturaheading}</div>
            </div>
            <div className="">
              <h1 className="flex justify-center font-bold text-5xl text-gray-800"><b>Join!</b></h1>
              <div className="flex space-x-8 justify-center pt-6">
                {Join}
              </div>
            </div>
            <div className="flex justify-center pt-16 pb-4 text-xl font-semibold">
              {c_backToTop}
            </div>
          </PageLayout>
        </AnalyticsScopeProvider>
